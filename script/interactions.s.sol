// SPDX-License-Identifier: MIT
 ...
 contract CreateSubscription is Script {
    ...
    function createSubscription(address vrfCoordinator, uint256 deployerKey) public returns (uint64) {
        ...
        console.log("Updated subscriptionId in HelperConfig contract.");
        return subId;
    }
 ...
 contract FundSubscription is Script {
    uint96 public constant FUND_AMMOUNT = 3 ether;
    ...
    function fundSubscription(address vrfCoordinator, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if(block.chainid == 31337){
            vm.startBroadcast(deployerKey);
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(subId, FUND_AMMOUNT);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(deployerKey);
            LinkToken(link).transferAndCall(vrfCoordinator, FUND_AMMOUNT, abi.encode(subId));
            vm.stopBroadcast();
        }
    }
